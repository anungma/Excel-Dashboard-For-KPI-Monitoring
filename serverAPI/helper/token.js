...
async function check(token, reply) {
    ...
    return new Promise((resolve) =>
        connection.query(sql, [token], function (error, rows) {
            ...
            if(rows.length > 0){
                let message = moment(rows[0].updated_at).format('YYYY-MM-DD HH:mm:ss').toString() > now;
                if(message){
                    return resolve({ user_id: rows[0].user_id, secret_id: rows[0].secret_id, token :  rows[0].id });
                }
                else{
