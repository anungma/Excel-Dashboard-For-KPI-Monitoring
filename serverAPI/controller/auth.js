...
async function createToken (request, reply) {
    ...
    let sql = `INSERT INTO web_authentication (id, user_id, secret_id, expires_at, created_at, updated_at)
      values(?, ?, ?, ?, ?, ?)`;
...
async function checkToken (request, reply) {
    ...
    let sql = `SELECT authentication.*, users.remember_token FROM web_authentication 
    INNER JOIN web_users ON web_users.id = web_authentication.user_id WHERE authentication.id = ? AND users.remember_token = ?`;
    let data = await new Promise((resolve) =>
